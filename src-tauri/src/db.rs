...
impl Database {
    pub fn get_vaults(&self, user_id: i32) -> Result<Vec<Vault>, String> {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, user_id, name_encrypted, color, image, nonce, created_at FROM vaults WHERE user_id = ? ORDER BY created_at DESC"
    ...
    pub fn create_vault(&self, user_id: i32, name: &str, color: &str) -> Result<Vault, String> {
        ...
        let key = self.get_encryption_key(user_id)?;
        let (name_encrypted, nonce) = encrypt_to_base64(name, &key)?;

        conn.execute(
            "INSERT INTO vaults (id, user_id, name_encrypted, color, nonce, created_at) VALUES (?, ?, ?, ?, ?, ?)",
            [&id, &user_id.to_string(), &name_encrypted, color, &nonce, &created_at.to_string()],
        ).map_err(|e| e.to_string())?;
    ...
    pub fn update_vault(&self, vault: &Vault) -> Result<(), String> {
        ...
        let (name_encrypted, nonce) = encrypt_to_base64(&vault.name, &key)?;

        conn.execute(
            "UPDATE vaults SET name_encrypted = ?, color = ?, nonce = ? WHERE id = ?",
            [&name_encrypted, &vault.color, &nonce, &vault.id],
        ).map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    pub fn get_vault(&self, vault_id: &str) -> Result<Option<Vault>, String> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare(
            "SELECT id, user_id, name_encrypted, color, image, nonce, created_at FROM vaults WHERE id = ?"
        ).map_err(|e| e.to_string())?;
    ...
    pub fn create_note(&self, vault_id: &str, title: &str, content: &str, user_id: i32) -> Result<Note, String> {
        ...
        let key = self.get_encryption_key(user_id)?;
        let (title_encrypted, nonce) = encrypt_to_base64(title, &key)?;
        let (content_encrypted, _) = encrypt_to_base64(content, &key)?;

        conn.execute(
            "INSERT INTO notes (id, vault_id, title_encrypted, content_encrypted, nonce, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
            [&id, vault_id, &title_encrypted, &content_encrypted, &nonce, &created_at.to_string(), &updated_at.to_string()],
        ).map_err(|e| e.to_string())?;
    ...
    pub fn update_note(&self, note_id: &str, title: &str, content: &str, user_id: i32) -> Result<(), String> {
        ...
        conn.execute(
            "UPDATE notes SET title_encrypted = ?, content_encrypted = ?, nonce = ?, updated_at = ? WHERE id = ?",
            [&title_encrypted, &content_encrypted, &nonce, &updated_at.to_string(), note_id],
        ).map_err(|e| e.to_string())?;
    ...
    pub fn get_note_with_content(&self, note_id: &str, user_id: i32) -> Result<Option<Note>, String> {
        ...
        let (id, vault_id, title_encrypted, content_encrypted, nonce, created_at, updated_at): (String, String, String, String, String, i64, i64) = conn.query_row(
            "SELECT n.id, n.vault_id, n.title_encrypted, n.content_encrypted, n.nonce, n.created_at, n.updated_at
             FROM notes n
             WHERE n.id = ?",
            [note_id],
            |row| Ok((
        ...
        let key = self.get_encryption_key(user_id)?;
        let title = decrypt_from_base64(&title_encrypted, &nonce, &key)?;
    ...
    pub fn get_notes_decrypted(&self, vault_id: &str, user_id: i32) -> Result<Vec<Note>, String> {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, vault_id, title_encrypted, content_encrypted, nonce, created_at, updated_at FROM notes WHERE vault_id = ? ORDER BY updated_at DESC"
        ).map_err(|e| e.to_string())?;
        ...
        for note_data in note_iter {
            let (id, vault_id, title_encrypted, content_encrypted, nonce, created_at, updated_at): (String, String, String, String, String, i64, i64) = note_data.map_err(|e| e.to_string())?;
            let title = decrypt_from_base64(&title_encrypted, &nonce, &key)?;
            let content = decrypt_from_base64(&content_encrypted, &nonce, &key)?;
            result.push(Note {
                id,
...
fn vault_from_row(row: &Row, user_id: i32, key: &GenericArray<u8, U32>) -> Result<Vault, rusqlite::Error> {
    ...
    let name = match decrypt_from_base64(&name_encrypted, &nonce, key) {
        Ok(n) => n,
